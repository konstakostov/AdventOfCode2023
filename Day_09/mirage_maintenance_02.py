...
for history in oasis:
    ...
    for row in range(len(history) - 1, -1, -1):
        ...
        if row == len(history) - 1:
            history[row].insert(0, 0)

        # For other row the new element added to the end of each row is calculated
        else:
            # The new element is equal to the next row's first element
            # subtracted from the current row's first element
            new_element = history[row][0] - history[row + 1][0]
            history[row].insert(0, new_element)

    # Adding the extrapolated value to 'extrapolated_values_sum'
    # The extrapolated value is the first value from the first row
    # of the current history sequence
    extrapolated_values_sum += (history[0][0])
